...
**Apple Health (Daily Steps):** fitr+ can optionally read your **daily step count** from Apple Health. This is entirely opt-in — the app never reads Apple Health unless you turn on "Sync daily steps" in your goals, and iOS asks your permission separately first. It is **read-only** (fitr+ never writes to Apple Health) and **steps only** (no other Health data type is requested). Step counts are read fresh each time a screen needs them — they are **not** copied into your account, not stored on our servers, and not part of the account sync above. If you also use the AI Coach, your recent daily step totals are included in the context sent to Google's Gemini API (see AI Features above) so guidance can account for your general daily activity; this is used only to provide fitness guidance inside fitr+. Health data obtained through Apple Health is **never** used for advertising, marketing, or use-based data mining, and is never sold or shared with data brokers. Turning the toggle off stops all step reading immediately and removes step data from everywhere it appears in the app.
...
| Apple Health (HealthKit) | Reads your daily step count, opt-in and read-only, to show alongside your progress | Nothing is sent to Apple; data is read on-device only | [Link](https://www.apple.com/legal/privacy/) |
