...
**Description:** fitr+ is a personal fitness tracking application — log workouts, track nutrition, monitor biometrics, set goals, and optionally use AI-powered features (Ask Coach, AI Meal Recipes, the AI Workout Plan Builder — powered by Google's Gemini model through our own backend, no API key of your own required). The App requires **a DataCraft Studio account (see General → Section 2) and an active App Membership** (below) — there is no free tier. The account keeps your **Energy Points** balance — spent on AI coaching answers, AI meal recipes, and AI workout plans — safe across devices, and to sync your workouts, nutrition logs, biometrics, goals, and profile so they survive a reinstall or a new device.
...
**Third-Party API Services.** When you use Ask Coach, AI Meal Recipes, or the AI Workout Plan Builder, your context and prompt are sent from your device to our own backend (Firebase Cloud Functions), which forwards them to Google's Gemini API using a key we manage and returns the response to your device — we do not store or log your prompts or Gemini's responses. You're responsible for complying with Google's terms for content generated this way, and your use is governed by [Google's Terms of Service](https://policies.google.com/terms). Typing into Food Search runs against a food database built into the App plus any foods you enter manually — no external service is involved. **Scanning a barcode does call one:** the barcode number is sent to [Open Food Facts](https://world.openfoodfacts.org), an independent, community-maintained open database, to look up the product. **That nutrition information is contributed by the public and is not verified by us** — it may be wrong, out of date, or missing, and package formulations change, so check it against the label before relying on it; every field stays editable before you add the food. Product data from Open Food Facts is published under the Open Database License and your use of it is governed by [their terms](https://world.openfoodfacts.org/terms-of-use). We're not responsible for outages, rate limits, or content from Google's Gemini API or Open Food Facts.

**Energy Points.** AI coaching answers, AI meal recipes, and AI workout plans each consume Energy Points from a server-managed balance tied to your account; manual logging (workouts, nutrition, biometrics) never costs points. Energy Points are deducted the moment you start an AI action and are **non-refundable**, even if the request fails, is interrupted, or you leave or background the App before it finishes — the App warns you of this before each AI action starts, so keep it open in the foreground until it completes. Points may be **earned free** — every new account receives a one-time welcome bonus, and you earn one point on any day you log a workout — or purchased as one-time in-app packages. Membership does **not** grant Energy Points (see below). Energy Points have no cash value, are non-transferable, and are not redeemable for cash.

**App Membership (Auto-Renewing Subscription).** **fitr+ requires an active App Membership to use the app** — there is no free tier and no guest mode. After creating an account and completing setup, you must start a membership (which begins with a free trial) before any part of the app becomes usable. Two plans, one at a time: **Monthly at $6.99/month** or **Annual at $49.99/year** (equivalently $4.17/month). **Both begin with a 14-day free trial**; if you don't cancel at least 24 hours before it ends, it converts automatically to a paid subscription. Membership unlocks the whole app — tracking, Trends, the weekly report, reminders, the rest timer, Apple Health step syncing, and access to the AI features — but **does not include Energy Points**, which are a separate currency (above). Membership is a standard Apple in-app auto-renewing subscription:

- Payment is charged to your Apple ID account at confirmation of purchase, or at the end of the free trial.
- If Apple refunds a purchase, your access ends when Apple notifies us, which may be before the period you originally paid for would have ended.
...
- You can manage or cancel your subscription anytime in your **Apple ID Account Settings** (Settings → Apple Account → Subscriptions on your device) — not through the App itself.
